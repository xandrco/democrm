        ...
        <>
            ...
            <div className="md:hidden space-y-3">
                {applications.map((application) => (
                    <div 
                        key={application.id} 
                        className="bg-white border-b md:border border-slate-200 overflow-hidden"
                    >
                        <div className="p-4">
                            <div className="flex justify-between items-start">
                                ...
                                {application.comments_count > 0 && (
                                    <div className="flex items-center text-xs text-slate-600 bg-slate-100 px-2 py-1 rounded-full">
                                        <svg className="h-3.5 w-3.5 mr-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                                        </svg>
                                        {application.comments_count}
                                    </div>
                            ...
                            <div className="mt-3 grid grid-cols-2 gap-3">
                                <div>
                                    <p className="text-xs text-slate-500 mb-1">Дата создания</p>
                                    <p className="text-sm text-slate-700">{formatDate(application.created_at)}</p>
                                </div>
                                <div>
                                    <p className="text-xs text-slate-500 mb-1">Дата обновления</p>
                                    <p className="text-sm text-slate-700">{application.reviewed_at ? formatDate(application.reviewed_at) : '—'}</p>
                                </div>
                            </div>
                            ...
                            <div className="mt-4">
                                <button 
                                    onClick={(e) => handleViewClick(e, application.id)}
                                    className="w-full bg-slate-100 rounded-md px-3 py-2 text-slate-700 hover:bg-slate-200 cursor-pointer text-center"
                                >
                                    Просмотр
                                </button>
                            </div>
